...
public class MavenResourceController {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(MavenResourceController.class);
	...
	public void stream(@PathVariable String groupId, @PathVariable String artifactId,
		...
		if (version.contains("exec")) {
			mavenResource = mavenResourceResolver
					.resolveUri(String.format("%s:%s:jar:%s:%s", groupId, artifactId,
							version.substring(version.lastIndexOf("-") + 1),
							version.substring(0, version.lastIndexOf("-"))));
		}
		else {
			mavenResource = mavenResourceResolver
					.resolveUri(String.format("%s:%s:%s", groupId, artifactId, version));
		}

		response.addHeader("Content-disposition",
				String.format("attachment;filename=%s", mavenResource.getFilename()));
		response.setContentType("application/java-archive");
		IOUtils.copy(new FileInputStream(mavenResource.getFile()),
